...
export const TestimonialsSection: React.FC = () => {
  ...
  return (
    <section className="relative w-full bg-[#91C6F2] overflow-hidden">
      {/* 
        The main white card has rounded-bl-[40px] lg:rounded-bl-[80px] at the bottom-left corner,
        revealing the light blue background (#91C6F2) of the CalculateSection below it seamlessly.
      */}
      <div className="w-full bg-white rounded-bl-[40px] lg:rounded-bl-[80px] pt-[40px] lg:pt-[60px] pb-[80px] lg:pb-[120px] px-[20px] lg:px-[60px]">
        <div className="max-w-[1320px] mx-auto flex flex-col items-center">
          ...
          >
            <div 
              className="flex w-full transition-transform duration-[1100ms] ease-[cubic-bezier(0.25,1,0.5,1)]"
              style={{ transform: `translateX(-${currentIndex * 100}%)` }}
            >
              {testimonialsData.map((item, idx) => (
                <div 
                  key={`testimonial-slide-${idx}`}
                  className="w-full shrink-0 flex flex-col lg:flex-row gap-[32px] lg:gap-[60px] items-center lg:items-start"
                >
                  {/* Left Image Side */}
                  <div className="relative w-full lg:w-[480px] aspect-[4/3] rounded-[20px] overflow-hidden shrink-0 shadow-[0_8px_30px_rgba(0,0,0,0.06)]">
                    <Image 
                      src={item.image} 
                      alt={item.name} 
                      fill 
                  ...
                  {/* Right Content Side */}
                  <div className="flex flex-col justify-center flex-1 min-h-[300px]">
                    <p className="font-secondary font-normal text-[16px] lg:text-[20px] leading-[1.5] text-[#111111] mb-[32px] lg:mb-[40px]">
                      {item.quote}
                    </p>

                    <div className="flex flex-col gap-[4px] lg:gap-[6px]">
                      <span className="font-primary font-normal text-[24px] lg:text-[32px] text-electric-blue leading-[1.2]">
                        {item.name}
                      </span>
                      <span className="font-secondary font-normal text-[14px] lg:text-[16px] text-[#7D8690]">
                        {item.role}
                      </span>
                    </div>
                  </div>
          ...
          <div className="flex items-center justify-between w-full mt-[32px] lg:mt-[48px] px-[8px] lg:px-[16px]">
            {/* Dots */}
            <div className="flex items-center gap-[8px]">
              {testimonialsData.map((_, i) => (
                <button
                  ...
                  className={`h-[6px] lg:h-[7px] rounded-[32px] transition-all duration-300 cursor-pointer p-0 border-none outline-none ${
                    currentIndex === i 
                      ? 'w-[32px] lg:w-[40px] bg-electric-blue' 
                      : 'w-[10px] lg:w-[12px] bg-[#91C6F2]'
                  }`}
                />
              ))}
            </div>
            ...
            <div className="flex items-center gap-[12px]">
              ...
              <button 
                ...
                className={`group w-[40px] h-[40px] lg:w-[48px] lg:h-[48px] rounded-full border border-electric-blue flex items-center justify-center transition-all duration-300 cursor-pointer focus:outline-none ${
                  currentIndex === 0 
